...
class UserRoleSignalTest(TestCase):
    def test_create_student_profile(self):
        user = User.objects.create(username="@student_user", email="student@example.com", role="student")
        self.assertTrue(Student.objects.filter(user=user).exists())

    def test_create_tutor_profile(self):
        user = User.objects.create(username="@tutor_user", email="tutor@example.com", role="tutor")
